...
const ProfileTab: React.FC<ProfileTabProps> = ({ userId, universityId }) => {
  ...
  // Handle bookmarked posts navigation
  const handleBookmarkedPosts = useCallback(() => {
  ...
  return (
    <div className="profile-tab">
      ...
      <ProfileActions
        ...
        onBookmarkedPosts={handleBookmarkedPosts}
      ...
      {/* Bookmarked Posts Modal */}
      {showBookmarkedPosts && (
        <BookmarkedPostsModal
          userId={userId}
  ...
  return (
    <div className="profile-header">
      ...
      <div className="profile-header-actions">
        <button className="settings-button" onClick={onAccountSettings}>
...
const ProfileActions: React.FC<ProfileActionsProps> = ({
  ...
  return (
    <div className="profile-actions">
      ...
      <button className="action-button bookmarked" onClick={onBookmarkedPosts}>
        <span className="icon">🔖</span>
        <span className="label">Bookmarked Posts</span>
      </button>
...
### **7. Bookmarked Posts Modal Component**
```typescript
interface BookmarkedPostsModalProps {
  userId: string;
  onClose: () => void;
}

const BookmarkedPostsModal: React.FC<BookmarkedPostsModalProps> = ({
  userId,
  onClose
}) => {
  ...
  const fetchBookmarkedPosts = useCallback(async () => {
    try {
      setLoading(true);
      const response = await getUserBookmarkedPosts(userId);
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content bookmarked-posts-modal" onClick={e => e.stopPropagation()}>
        <div className="modal-header">
        ...
        <div className="modal-body">
          {loading ? (
            ...
            <div className="error-state">
              ...
              <p>{error}</p>
              <button onClick={fetchBookmarkedPosts}>Try Again</button>
            </div>
            ...
            <div className="bookmarked-posts-list">
              {bookmarkedPosts.map(post => (
                <div key={post.id} className="bookmarked-post-card">
                  <div className="post-header">
                    <div className="post-type-badge">
                      {post.postType === 'offer' && <span className="badge offer">Offer</span>}
                      {post.postType === 'request' && <span className="badge request">Request</span>}
                  ...
                  <div className="post-meta">
                    <span className="post-author">by {post.user.displayName}</span>
...
.bookmarked-post-card .post-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
  margin-bottom: var(--space-3);
...
.bookmarked-post-card .post-title {
  font-size: var(--text-lg);
  font-weight: var(--font-semibold);
  margin: 0 0 var(--space-2) 0;
  color: var(--text-primary);
...
.bookmarked-post-card .post-description {
  font-size: var(--text-base);
  color: var(--text-secondary);
  margin: 0 0 var(--space-3) 0;
  line-height: var(--leading-relaxed);
...
.bookmarked-post-card .post-tags {
  display: flex;
  gap: var(--space-2);
  margin-bottom: var(--space-3);
  flex-wrap: wrap;
...
.bookmarked-post-card .post-meta {
  display: flex;
  justify-content: space-between;
  align-items: center;
  margin-bottom: var(--space-3);
...
.bookmarked-post-card .post-actions {
  display: flex;
  gap: var(--space-2);
}

.bookmarked-post-card .action-button {
  flex: 1;
  padding: var(--space-2) var(--space-3);
  border: 1px solid var(--bg-tertiary);
  border-radius: var(--radius-md);
...
.bookmarked-post-card .action-button:hover {
  background-color: var(--bg-secondary);
  border-color: var(--primary-blue);
}

.bookmarked-post-card .action-button.contact {
  background-color: var(--primary-blue);
  color: var(--bg-primary);
  border-color: var(--primary-blue);
}

.bookmarked-post-card .action-button.contact:hover {
  background-color: var(--primary-blue-dark);
}
```
